...
Next, we consider the derivation of closed-form expression for the LASSO regression estimator of the regression coefficients $\beta$. According to equation (27) on the 'Regularized Regression' lecture slides, $\hat{\beta}_n^{\text{LASSO}}\equiv\text{ arg min }_{\beta \in \textbf{R}^J}\|\textbf{Y}_n-\textbf{X}_n\beta\|_2^2 + \lambda\|\beta\|_1=\text{ arg min }_{\beta \in \textbf{R}^J}\sum_{i=1}^n\big(Y_i - \sum_{j=1}^J\beta_jX_{i,j}\big)^2+ \lambda\sum_{i=1}^J|\beta_j|$
...
barplot(df$lambda_optimal, main = expression("Ridge: Optimal shrinkage parameter for each covariate"), ylab = expression(lambda), col=cm.colors(10), names.arg = rownames(df))
